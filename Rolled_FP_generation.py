...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/fingerprint-ldm-vq-128-512-rolled.yaml",
    ...
    parser.add_argument(
        "--ckpt",
        type=str,
        default="models/fingerprint_ldm_rolled_512/model_ldm_rolled.ckpt",
    ...
    parser.add_argument( # no used, direct generate
        "--strength",
        type=float,
        default=0.75,
    ...
    sample_path = os.path.join(outpath, 'raw_rolled') # folder_type
    os.makedirs(sample_path, exist_ok=True)
    iters = opt.nums // batch_size + 1
                ...
                with model.ema_scope("Plotting"): # testing
                    ...
                    for j, x_sample in enumerate(x_samples):
                        ...
                        Image.fromarray(x_sample.astype(np.uint8)).save(
                            os.path.join(sample_path, f"{i*batch_size+j:05d}.png"))
    print(f"All samples are saved to {sample_path}")
